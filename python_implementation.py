...
def substitute_var(e: Expr, old_name: str, new_name: str) -> Expr:
    """Substitute variable names to avoid capture."""
    if isinstance(e, Var):
        if e.name == old_name:
            return Var(new_name)
        else:
...
def substitute(e1: Expr, x: str, e2: Expr) -> Expr:
    ...
    if isinstance(e1, Var):
        ...
        if x1 == x:
            # x1 is bound, x is shadowed
            tau2 = e1.tau2
        elif x1 not in free_vars(e2):
            # Safe to substitute in tau2
            tau2 = substitute(e1.tau2, x, e2)
        else:
            # Need to rename x1 to avoid capture
